...
resource "aws_ecs_service" "main" {
  ...
  lifecycle {
    # Ignore desired_count changes made by autoscaling between Terraform runs
...
resource "aws_appautoscaling_policy" "cpu" {
  ...
  name               = "${var.project}-${var.environment}-${var.service_name_suffix}-cpu-scaling"
...
resource "aws_appautoscaling_policy" "memory" {
  ...
  name               = "${var.project}-${var.environment}-${var.service_name_suffix}-memory-scaling"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.ecs[0].resource_id
  scalable_dimension = aws_appautoscaling_target.ecs[0].scalable_dimension
  service_namespace  = aws_appautoscaling_target.ecs[0].service_namespace
